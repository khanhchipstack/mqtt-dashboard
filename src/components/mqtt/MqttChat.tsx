...
interface MqttChatBoxProps {
  ...
  isConnected: boolean;
  protocolVersion: 4 | 5;
  selectedTopic: SubscribeOptions | null;
}
...
const MqttChatBox: React.FC<MqttChatBoxProps> = ({
  ...
  const handlePublish = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTopic) return toast.error("Select a topic to publish.");
    if (!payload.trim()) return toast.error("Payload cannot be empty.");

    // Validate theo định dạng
    if (format === "json") {
      try {
        JSON.parse(payload);
      } catch {
    ...
    if (enableProperties && userProperties) {
      try {
        ...
        return toast.error("Invalid JSON for User Properties.");
      }
    }
